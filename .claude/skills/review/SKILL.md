...
- **Implied ADR Formalization**: Review any temporary files in `.claude/reports/tmp/implied-adr/implied-adr-*.md`. Determine if these decisions should be promoted to a formal ADR, modified, or discarded.
...
The output must be an actionable Markdown checklist formatted as `.claude/reports/ql-report-latest.md`. Use `[ ]` for each finding so the human can easily triage and check `[x]` the ones they want to fix.
